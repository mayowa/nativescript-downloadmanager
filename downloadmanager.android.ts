...
export class DownloadManager {
  ...
  public constructor() {
    this.manager = Application.android.context.getSystemService(
  ...
  public downloadFile(
    url: string,
    options: definition.DownloadOptions,
    cb: Function
  ) {
    ...
    if (options.description) req.setDescription(options.description);
    ...
    if (options.allowScanningByMediaScanner) req.allowScanningByMediaScanner();

    if (options.disallowOverMetered) req.setAllowedOverMetered(false);

    if (options.disallowOverRoaming) req.setAllowedOverRoaming(false);

    if (options.mimeType) req.setMimeType(options.mimeType);

    if (options.notificationVisibility != undefined)
      req.setNotificationVisibility(options.notificationVisibility);

    if (options.hideInDownloadsUi) req.setVisibleInDownloadsUi(false);
  ...
  private handleDownloadEvent(
    ...
    while (c.moveToNext()) {
      if (
        c.getLong(c.getColumnIndex(android.app.DownloadManager.COLUMN_ID)) == id
      ) {
        var success;
        switch (
        ...
        if (this.downloads.has(id)) {
          let uri = c.getString(
            c.getColumnIndex(android.app.DownloadManager.COLUMN_LOCAL_URI)
          );
